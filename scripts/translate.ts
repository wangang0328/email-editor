...
const overwriteData = fs.readJsonSync(
  path.join(process.cwd(), 'packages/email-editor-localization/locales/overwrite.json'),
);
...
async function main() {
  ...
  Object.keys(translateWords).forEach(lan => {
    ...
    fs.writeFileSync(
      `packages/email-editor-localization/locales/${lan}.json`,
      JSON.stringify(combineData, null, 2),
    );
  });

  fs.writeFileSync(
    `packages/email-editor-localization/locales/locales.json`,
